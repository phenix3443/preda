...
bool Simulator::Deploy(ExecutionUnit& exec_units, const rt::String_Ref* fns, const rt::String_Ref* ctor_args, uint32_t co, bool within_script, rvm::InvokeResult &ctor_invoke_result)
{
	...
	auto check = [this](BuildContracts& b) {
		if (b.GetSourceCount() == 0)return true;
		for (uint32_t i = 0; i < b.GetSourceCount(); i++) {
			_LOG_WARNING("[PRD]: Unsupported target for source code: " << i);
			const rvm::Contract* compiled_contract; 
			return CheckDeployedContract(compiled_contract);
		}
		return true;
	};
...
bool Simulator::CheckDeployedContract(const rvm::Contract* compiled_contract) 
{
	const rvm::Contract* deployed_contract = GetDeployedContract(compiled_contract);
	if (!deployed_contract){
		return true;
	}
	return CheckContractStateVariables(compiled_contract, deployed_contract) && 
		CheckContractCustomTypes(compiled_contract, deployed_contract) && 
		CheckContractExportedFunctions(compiled_contract, deployed_contract);
}
...
bool Simulator::CheckContractStateVariables(const rvm::Contract* compiled_contract, const rvm::Contract* deployed_contract) 
{
	// TODO
	return true;
}

bool Simulator::CheckContractCustomTypes(const rvm::Contract* compiled_contract, const rvm::Contract* deployed_contract) 
{
	// TODO
	return true;
}

bool Simulator::CheckContractExportedFunctions(const rvm::Contract* compiled_contract, const rvm::Contract* deployed_contract)
{
	// TODO
	return true;
}
